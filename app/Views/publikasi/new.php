...
<form action="<?= site_url('publikasi') ?>" method="post" autocomplete="off" enctype="multipart/form-data">
    ...
    <input type="hidden" name="jenis_pengabdian_atau_penelitian" id="jenis_pengabdian_atau_penelitian" value="<?= url(3) ?>">
                ...
                <div class="col-md-4">
                    <div class="form-group mb-0">
                        ...
                        <select name="status_akreditasi_jurnal" id="status_akreditasi_jurnal" class="form-control <?= isset(session('errors')['status_akreditasi_jurnal']) ? 'is-invalid' : '' ?>">
                            <option value="0" <?= old('status_akreditasi_jurnal') == '0' ? 'selected' : '' ?>>Tidak Terakreditasi</option>
                            <option value="1" <?= old('status_akreditasi_jurnal') == '1' ? 'selected' : '' ?>>Terakreditasi</option>
                        </select>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group mb-0">
                        ...
                        <select name="sinta" id="sinta" class="form-control <?= isset(session('errors')['sinta']) ? 'is-invalid' : '' ?>">
                            ...
                            <?php foreach(['C1','C2','C3','C4','C5','C6'] as $s): ?>
                                <option value="<?= $s ?>" <?= old('sinta') == $s ? 'selected' : '' ?>>Sinta <?= $s ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
    ...
    <div class="row">
        <div class="col-md-3">
            <div class="form-group">
                ...
                <input type="date" name="tanggal_terbit" id="tanggal_terbit" class="form-control <?= isset(session('errors')['tanggal_terbit']) ? 'is-invalid' : '' ?>" value="<?= old('tanggal_terbit') ?>">
                <div class="invalid-feedback">
