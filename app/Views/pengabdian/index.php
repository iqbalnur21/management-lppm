...
<tbody>
    <?php if (empty($pengabdian_data)) : ?>
        <tr>
            <td colspan="7" class="text-center">Tidak ada data pengabdian yang ditemukan.</td>
        </tr>
    <?php else : ?>
        <?php foreach ($pengabdian_data as $key => $value) : ?>
            <tr>
                <td><?= $key + 1 ?></td>
                <td><?= esc($value['nomor_surat'] ?? 'N/A') ?></td>
                <td><?= esc($value['judul_pengabdian']) ?></td>
                <td><?= esc($value['nama_lengkap']) // Dari JOIN ke tabel users ?></td>
                ...
                <td class="text-center" style="width: 200px;">
                    <a href="<?= site_url('pengabdian/' . $value['id_pengabdian']) ?>" class="btn btn-info btn-sm"><i class="fas fa-eye"></i> Detail</a>
                    
                    <?php if ($value['status'] == 'menunggu' || $value['status'] == 'revisi') : ?>
                        <a href="<?= site_url('pengabdian/' . $value['id_pengabdian'] . '/edit') ?>" class="btn btn-warning btn-sm"><i class="fas fa-pencil-alt"></i> Edit</a>
